...
import ValidMoveMarker from './ValidMoveMarker';
...
const ChessBoard: React.FC = () => {
  ...
  const renderPiece = (piece: string) => {
    if (!piece) return null; // Return null if piece is empty
    
    // Use type assertion to assert the piece type
    const pieceKey = piece as PieceType;
  
    // Check if pieceKey is a valid key in pieceImages
    if (pieceImages[pieceKey]) {
      return <img src={pieceImages[pieceKey]} alt={piece} className="chess-piece" />;
    }
  
    return null; // Return null if piece is not valid
  };
...
//   Function to validate if a move is legal
  const isValidMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
  ...
  const isValidPawnMove = (piece: string, startRow: number, startCol: number, endRow: number, endCol: number) => {
  ...
  const isValidRookMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
    if (startRow !== endRow && startCol !== endCol) return false; // Must move in straight lines
    return isPathClear(startRow, startCol, endRow, endCol);
  };

  const isValidKnightMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
    const rowDiff = Math.abs(startRow - endRow);
    const colDiff = Math.abs(startCol - endCol);
    return (rowDiff === 2 && colDiff === 1) || (rowDiff === 1 && colDiff === 2);
  };

  const isValidBishopMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
    if (Math.abs(startRow - endRow) !== Math.abs(startCol - endCol)) return false; // Must move diagonally
    return isPathClear(startRow, startCol, endRow, endCol);
  };

  const isValidQueenMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
    return isValidRookMove(startRow, startCol, endRow, endCol) || isValidBishopMove(startRow, startCol, endRow, endCol);
  };

  const isValidKingMove = (startRow: number, startCol: number, endRow: number, endCol: number) => {
    const rowDiff = Math.abs(startRow - endRow);
    const colDiff = Math.abs(startCol - endCol);
  ...
  return (
    <div className="chess-container">
      <div className="timers">
        <div className={`timer ${isWhiteTurn ? 'active' : ''}`}>White: {formatTime(whiteTime)}</div>
        <div className={`timer ${!isWhiteTurn ? 'active' : ''}`}>Black: {formatTime(blackTime)}</div>
      </div>
      
      <div className="chessboard">
        {board.map((rowArray, rowIndex) => (
          <div key={rowIndex} className="chessboard-row">
            <div className="chessboard-label">{rowLabels[rowIndex]}</div>
            {rowArray.map((piece, colIndex) => (
              <div
                ...
                className={`chessboard-square ${
                  ...
                  validMoves.some(move => move.row === rowIndex && move.col === colIndex) ? <ValidMoveMarker position={{ row: rowIndex, col: colIndex }} /> : ''
                }`}
        ...
        <div className="chessboard-row">
          ...
          {columnLabels.map((label, index) => (
            <div key={index} className="chessboard-label">{label}</div>
          ))}
        </div>
      </div>
